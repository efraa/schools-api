export const UserResponses = {
  VALIDATOR: {
    NAME: 'Please, enter a name with 3 or more characters.',
    USERNAME: 'Please, enter a username with 3 or more characters.',
    EMAIL: 'Please, include a valid email.',
    PASSWORD: 'Please, enter a password with 6 or more characters.',
    COMMON_PASSWORD: 'Oops!, your password is not safe, try using not so common words and numbers.',
  },
  USER_NOT_FOUND: 'Oops! We did not find this user in the organization.',
  UNAUTHORIZED: 'Oops! You are not authorized for this request.',
  NO_USER_DATA: 'Oops! We haven\'t found any users in your organization.',
  USERNAME_EXISTS: 'Oops! Username is already registered',
  EMAIL_EXISTS: 'Oops! Email is already registered',
  STATUS: 'Oops! This user account is ',
  DELETE: 'the user has been removed.',
}
